...
class MegatronPPOActor(BasePPOActor):
    ...
    def __init__(
        ...
        if self.use_torch_profiler:
            self.prof = Profiler(self.config.profiler, tool_config=self.config.profiler.get("tool_config", {}).get("torch", {}))
        else:
            self.prof = None
    ...
    def forward_backward_batch(
        ...
        if use_dynamic_bsz:
            ...
            if vpp_size is not None and vpp_size > 1:
                ...
                assert len(micro_batches) % self.tf_config.microbatch_group_size_per_vp_stage == 0, (f"micro_batches {micro_batches} must be divisible by microbatch_group_size_per_vp_stage "
            ...
            if self.use_fused_kernels:
                ...
                def logits_processor(logits, label, label_mask):
                    ...
                    if calculate_entropy:
                        ...
                        logger.warning_once("For memory-efficient computation, enable fused kernels via "
        ...
        if mpu.get_pipeline_model_parallel_world_size() > 1:
            ...
            # losses_reduced: [[{}, tensor]]
            if hasattr(self, 'iteration') == False:
                self.iteration = 0
            save_path = f'/data/logs/megatron-dump-gpu/megatron_actor_forward_bachward_batch_0_iter{self.iteration}_rank{torch.distributed.get_rank()}.pt'
            # torch.save({'losses_reduced': losses_reduced}, save_path)
            if torch.distributed.get_rank() == 0:
                breakpoint()
            cache_value = torch.load(save_path, map_location=losses_reduced[0][1].device, weights_only=False)
            losses_reduced = cache_value['losses_reduced']
            self.iteration += 1
