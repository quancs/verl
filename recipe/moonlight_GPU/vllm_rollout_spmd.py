...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            #     idx.device
            # )
            # torch.save(response, f'/data/logs/response-rank-{torch.distributed.get_rank()}-iter{self.iteation}.pth')
            # response=torch.load(f'/data/logs/response-rank-{torch.distributed.get_rank()}-iter{self.iteation}.pth').to(idx.device)
            self.iteration+=1
