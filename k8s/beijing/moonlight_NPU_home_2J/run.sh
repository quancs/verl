...
TEST_FILE=/data/datasets/aime-2024.parquet
...
cd /opt/verl
ray job submit --runtime-env-json='{"working_dir": ".", "excludes": ["/.git/", "/model_ckpts/", "/logs/", "/k8s", "/docker", "/doc", "/tmp", "/kernel_meta", "/plog"], "env_vars": {"VLLM_USE_V1": "1"}}' \
    -- python3 -m recipe.dapo.main_dapo \
    --config-path=config \
    --config-name="dapo_megatron_trainer" \
    hydra.run.dir=$JOB_LOG_DIR_CURR \
    ...
    trainer.device="npu" $@ 2>&1 | tee $JOB_LOG_DIR_CURR/run.log
